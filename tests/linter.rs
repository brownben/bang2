...
macro_rules! bang_lint {
  ($name:ident $code:literal $($rule:literal [$($num:literal)*])*) => {
    #[test]
    fn $name() {
      match bang::parse($code) {
        Ok(ast) => {
          let warnings = bang::lint(&ast);
          $({
            let warning = warnings.iter().find(|warning| warning.name == $rule).unwrap();
            assert_eq!(warning.issues.iter().map(|token| token.line).collect::<Vec<u16>>(), vec![$($num),*]);
          };)*
        }
